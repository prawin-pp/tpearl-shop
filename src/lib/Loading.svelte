...
<div
  id="loading"
  class="fixed inset-0 flex flex-col items-center justify-center bg-black bg-opacity-50 transition-all"
  class:opacity-0={$loading <= 0}
  class:opacity-100={$loading > 0}
  class:-z-[1]={$loading <= 0}
  class:z-[9999]={$loading > 0}
>
  <svg
    aria-hidden="true"
    class="mr-2 h-20 w-20 animate-spin fill-teal-600 text-gray-200 dark:text-gray-600"
    viewBox="0 0 100 101"
    fill="none"
  ...
  <div class="mt-10 animate-pulse text-3xl text-white">กรุณารอสักครู่</div>
</div>
