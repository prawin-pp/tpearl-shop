...
{#if visible}
  <div
    class="fixed inset-0 flex flex-col items-center justify-center gap-y-5 overflow-auto bg-gray-100"
  >
    <div
    ...
    <div
      class="cursor-pointer rounded-xl p-4 text-[48px] text-cash hover:animate-pulse"
      on:click={() => dispatch('select-payment-channel', 'CASH')}
    >
      {paymentChannelText.CASH}
    </div>
    <div
      class="cursor-pointer rounded-xl p-4 text-[48px] text-promptpay hover:animate-pulse"
      on:click={() => dispatch('select-payment-channel', 'PROMPTPAY')}
    >
      {paymentChannelText.PROMPTPAY}
    </div>
    <div
      class="cursor-pointer rounded-xl p-4 text-[48px] text-grab hover:animate-pulse"
      on:click={() => dispatch('select-payment-channel', 'GRAB')}
    >
      {paymentChannelText.GRAB}
    </div>
    <div
      class="cursor-pointer rounded-xl p-4 text-[48px] text-lineman hover:animate-pulse"
      on:click={() => dispatch('select-payment-channel', 'LINEMAN')}
    >
      {paymentChannelText.LINEMAN}
    </div>
    <div
      class="cursor-pointer rounded-xl p-4 text-[48px] text-robinhood hover:animate-pulse"
